...
const zhCN: Record<string, string> = {
  ...
  'btn.viewLog': '查看日志',
  'env.ready': '{name} 已就绪',
  'env.nodeReady': 'Node {version}',
  'env.nodeOld': 'Node {version} 过低',
  'env.nodeHint': 'OpenClaw 要求 Node >=24.16 <25 或 >=26.1，请升级 Node 后再更新 OpenClaw。',
  'env.missing': '{name} 缺失',
  'status.idle': '就绪',
  ...
  'settings.sectionTextRender': '文字渲染',
  'settings.textRenderMode': '渲染方式',
  'settings.renderAuto': '跟随主题',
  'settings.renderNone': '无',
  'settings.renderOutline': '描边',
  'settings.renderShadow': '投影',
  'settings.renderBoth': '描边+投影',
  'settings.textStrength': '强度',
  'settings.textRenderHint': '浅色模式玻璃偏亮、桌面底色不可控，默认用黑色描边；深色模式默认用投影。觉得描边太重，就把强度调低或直接选「无」。',
  'settings.sectionMaterial': '窗口材质',
  'settings.material': '透明材质',
  'settings.materialNone': '无',
  'settings.materialAcrylic': '亚克力',
  'settings.materialMica': '云母',
  'settings.materialTabbed': '云母 Alt',
  'settings.materialBlur': '实时玻璃',
  'settings.materialHint': '「实时玻璃」是 DWM 的实时高斯模糊（不额外着色），观感最接近 macOS；「亚克力」在其上还叠了噪点与色调；两个「云母」只取壁纸色调、不模糊窗口后面的内容，且需要 Windows 11。默认「无」性能最好。',
...
const enUS: Record<string, string> = {
  ...
  'env.nodeReady': 'Node {version}',
  'env.nodeOld': 'Node {version} too old',
  'env.nodeHint': 'OpenClaw requires Node >=24.16 <25 or >=26.1. Upgrade Node before updating OpenClaw.',
  ...
  'settings.sectionTextRender': 'Text Rendering',
  'settings.textRenderMode': 'Mode',
  'settings.renderAuto': 'Follow theme',
  'settings.renderNone': 'None',
  'settings.renderOutline': 'Outline',
  'settings.renderShadow': 'Shadow',
  'settings.renderBoth': 'Outline + shadow',
  'settings.textStrength': 'Strength',
  'settings.textRenderHint': 'The light theme is a bright glass over an unpredictable desktop, so it outlines text in black by default; the dark theme uses a drop shadow. If the outline feels too heavy, lower the strength or pick None.',
  'settings.sectionMaterial': 'Window Material',
  'settings.material': 'Transparency material',
  'settings.materialNone': 'None',
  'settings.materialAcrylic': 'Acrylic',
  'settings.materialMica': 'Mica',
  'settings.materialTabbed': 'Mica Alt',
  'settings.materialBlur': 'Live glass',
  'settings.materialHint': 'Live glass is the DWM real-time Gaussian blur with no tint, the closest to the macOS look. Acrylic adds noise and a tint on top of it. The two Mica options only sample the wallpaper and do not blur what is behind the window, and they require Windows 11. None is the fastest.',
